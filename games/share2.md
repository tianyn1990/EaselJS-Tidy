...
1. 通过获取到的 canvas 上下文，来在画布上进行绘制。
```
var context = document.getElementById("canvas").getContext('2d');
```

2. context 是一个状态机，它的 API 有改变状态的、有进行具体绘制操作的。而绘制操作是基于 context 的当前状态。
```
//定义 context 状态
...
```
上述 canvas 操作，是通过控制 context 这只笔进行绘制，而非控制一个矩形对象。 

3. 动画的生成是不断 绘制、擦除、再绘制 的过程，如果 2 中的操作封装为函数 `drawRect(color, alpha, x, y, w, h)`，那么动画可以这样生成：
```
var x = 10, y = 10;
...
基于上述特质，决定它的学习门槛低、适合快速制作小项目、维护稳定、代码量少适合学习等。
并且已经有同学进行过学习甚至有上线的作品了。
因此学习它是比较划算的。

下面把其中的几个重点简要介绍一下，主要是学习过程中感觉比较 **惊喜** 的部分：
...
rect.graphics.f('#f60').dr(0,0,50,50);
...
//将矩形对象添加到 stage
stage.addChild(rect).set({x: 100, y: 100}).graphics.f('#f60').dc(0, 0, 50);
...
每个类除了可以用来初始化一个对象，如：`new createjs.Stage('myCanvas')` 外，有的也包含一些静态方法，如：`createjs.Ticker.setFPS(20)` 
...
那么当浏览器的刷新频率降低（有可能是性能原因，或者是我们手动变更了 fps 值），
...
Ticker 可以将动画与帧率解耦，动画的执行是基于时间的（而非频率），
来实现改变 FPS 时不会影响动画的速率。

也就是说，每一次循环中的「位移改变量」取决于时间，而不是帧率。当帧率下降的时候，每次循环中的位移量会相应变大。
...
另外，我们可以通过设置 Ticker 的 timgingMode 属性，来设置循环的策略：requestAnimationFrame、setTimeout，或相结合。

其它部分：继承 & 命中测试 & 鼠标交互 & 事件管理 等请参考[这里](https://github.com/tianyn1990/EaselJS-Tidy/blob/master/Tutorial.md#继承)

> 由于 EaselJS 的中文资料不多而且不完整，建了 [EaselJS-Tidy](https://github.com/tianyn1990/EaselJS-Tidy)
> 这个项目希望对 EaselJS API doc 边学习边整理，同时分析其结构与运行方法，方便其他初学者学习，也方便以后查阅。
...
假设两个采集点的坐标为 s2(x1, y1)、s3(x2, y2) ，小火车位于两个采集点之间比例为 q 的位置，就可以得到小火车当前的精确坐标值了。
...
然后通过上图的计算方式（线性内插），就可以得到小火车当前的角度值了。
...
2. 小火车在最高点翻转的时候（转了一圈），由于使用的 atan2 的取值范围为 [-PI/2, PI/2] ，因此需要特殊计算。
...
//计算当前小火车的角度值
if (Math.abs(preRot - nextRot) > 180) {
    if (preRot < nextRot) {
      preRot += 180 * 2
    } else {
      nextRot += 180 * 2
...
shape.rotation = q * nextRot + (1 - q) * preRot;
```
